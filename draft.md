...
线程是对函数（一般称之为线程主函数）的封装，线程的执行就是多个嵌套的函数在栈上进出，并执行相应的指令，所以线程的切换需要保存栈，以及切换时的栈的位置（栈顶指针）。协程是对异步函数的封装，异步函数的执行同样也需要栈，而栈必然与一个线程一一对应，所以可以认为一个协程运行在一个线程中。Rust没有直接在语言层面提供完成的协程机制，但提供了异步编程机制—只需要在普通函数的函数声明之前添加`async`关键字，即可创建异步函数，它的特点在于，编译器会把异步函数创建为一个状态机，它可以看作是由局部变量，状态指针和多个状态所对应的函数所组成的对象，在每个让出点让出时，相当于该状态的函数已经执行完成，栈上已不存在异步函数的内容（我们称之为让出栈），当它被再次唤醒执行时，会从下一个状态所对应的函数开始执行，直到下一个状态，所以异步函数因为主动让出引起的切换不需要保存上下文。当异步函数还未执行到让出点就遭遇外部事件（如时钟中断，其它线程的抢占等）而被迫中断时，由于协程是在线程中执行，所以此时可以使用线程的中断机制，协程作为线程的指令而被保存在栈中，待线程恢复时协程可以继续执行。
...
创建协程的接口`coroutine_create`要求传入一个异步函数，其中封装了这个协程需要执行的指令语句。Rust语言中，只需要在普通函数声明前添加async关键字，即可将函数声明为异步函数。协程创建接口会把传入的异步函数封装为协程，并插入协程队列，等待被调度执行。
